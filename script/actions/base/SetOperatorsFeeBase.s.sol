...
contract SetOperatorsFeeBaseScript is ScriptBase {
    function runBase(address feeRegistry, address vault, uint256 fee)
        public
        returns (bytes memory data, address target)
    {
        target = address(IFeeRegistry(feeRegistry));
        data = abi.encodeCall(IFeeRegistry(feeRegistry).setOperatorsFee, (vault, fee));

        sendTransaction(target, data);

        Logs.log(string.concat("Set operators fee", "\n    vault:", vm.toString(vault), "\n    fee:", vm.toString(fee)));
